...
net = ["NZ"]                                    # network  
sta = ["M?Z"]                                   # station (do either one station or *)
start_date = ["2018_05_01"]                     # start date of download
end_date   = ["2018_05_03"]                     # end date of download
inc_days   = 2                                  # length of data for each request

# time tags
starttime = obspy.UTCDateTime(start_date[0])       
endtime   = obspy.UTCDateTime(end_date[0])
all_chunck= noise_module.get_event_list(start_date[0],end_date[0],inc_days)

# assemble parameters for pre-processing
prepro_para = {'rm_resp':rm_resp,'respdir':respdir,'freqmin':freqmin,'freqmax':freqmax,\
    'samp_freq':NewFreq,'start_date':start_date,'end_date':end_date,'inc_days':inc_days}
...
for ick in range(len(all_chunck)-1):
    ...
    for ista in range(nsta):
        ...
        sta_inv = client.get_stations(network=net[ista],station=sta[ista],\
            channel=chan[0],starttime=s1,endtime=s2,location=location[ista],level="response")
        ...
        with pyasdf.ASDFDataSet(ff,compression="gzip-3") as ds:
            ...
            if len(tr):
                    new_tags = '{0}s{1}s{2}s{3}'.format(net[ista].lower(),sta[ista].lower(),\
