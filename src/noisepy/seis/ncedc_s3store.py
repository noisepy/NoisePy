...
class NCEDCS3DataStore(RawDataStore):
    """
    A data store implementation to read from a directory of miniSEED files from the NCEDC S3 bucket.
    ...
    def _load_channels(self, full_path: str, ch_filter: Callable[[Channel], bool]):
        ...
        for f in msfiles:
            timespan = NCEDCS3DataStore._parse_timespan(f)
            self.paths[timespan.start_datetime] = full_path
            channel = NCEDCS3DataStore._parse_channel(os.path.basename(f))
        ...
        if key not in self.channels or date_range.start_datetime not in self.paths:
            ...
            for d in range(0, dt.days):
                ...
                if self.date_range is None or date not in self.date_range:
                    continue
                date_path = str(date.year) + "/" + str(date.year) + "." + str(date.timetuple().tm_yday).zfill(3) + "/"
